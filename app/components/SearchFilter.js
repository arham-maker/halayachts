...
import { FiSearch, FiX, FiChevronDown, FiCheck, FiFilter } from 'react-icons/fi';
import { useSearchParams } from 'next/navigation';

// HTML constants for consistent styling
const DROPDOWN_STYLES = {
  container: "relative",
  button: "w-full px-4 py-3 text-left bg-white border border-gray-300 rounded-lg hover:border-gray-400 focus:border-text-secondary focus:ring-2 focus:ring-text-secondary transition-all duration-200 flex items-center justify-between",
  buttonText: "text-base leading-relaxed tracking-wider truncate",
  dropdown: "absolute top-full left-0 mt-2 bg-white border border-gray-300 rounded-lg shadow-lg z-50 max-h-80 overflow-hidden",
  searchContainer: "p-2 border-b border-gray-200",
  searchInput: "w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-text-secondary focus:border-text-secondary",
  optionsContainer: "max-h-60 overflow-y-auto",
  option: "w-full px-4 py-2 text-left hover:bg-gray-50 transition-colors duration-150 flex items-center justify-between",
  selectedOption: "bg-text-secondary-50 text-black",
  normalOption: "text-base leading-relaxed tracking-wider",
  clearButton: "text-base leading-relaxed tracking-wider w-full text-red-600 hover:text-red-700 py-1 flex items-center justify-center gap-1"
};
...
const FILTER_CONTAINER_STYLES = {
  ...
  clearAll: "text-base md:text-lg leading-relaxed tracking-wider font-medium text-secondary hover:text-primary flex items-center gap-1",
...
const MOBILE_STYLES = {
  ...
  sheet: "fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-xl z-50 max-h-[85vh] overflow-hidden md:hidden transition-transform duration-300 ease-in-out",
  sheetHeader: "flex items-center justify-between p-4 border-b border-gray-200 bg-white sticky top-0 z-10",
  ...
  applyButton: "w-full bg-text-secondary text-white py-4 rounded-lg font-semibold text-lg hover:bg-primary transition-colors mt-10 sticky bottom-4 z-10"
};
...
const FilterDropdown = ({ 
  ...
  return (
    <div className={DROPDOWN_STYLES.container}>
      ...
      {isOpen && (
        <div className={`${DROPDOWN_STYLES.dropdown} ${widthClass} ${isMobile ? 'relative mt-2 shadow-none border-0 max-h-48 overflow-y-auto' : ''}`}>
          {isSearchable && (
          ...
          <div className={`${DROPDOWN_STYLES.optionsContainer} ${isMobile ? 'max-h-32' : ''}`}>
              ...
              <button
                ...
                className={`${DROPDOWN_STYLES.option} ${
                  (isMultiSelect 
                    ...
                    : value === option.value
                  ) ? DROPDOWN_STYLES.selectedOption : DROPDOWN_STYLES.normalOption
                }`}
              >
...
const SearchFilter = ({ onFilterChange, className = '', showClearAll = true, initialFilters = {} }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    passengers: searchParams.get('passengers') || initialFilters.passengers || 'all',
    amenities: searchParams.getAll('amenities') || initialFilters.amenities || 'all'
  });
  ...
  const DesktopFilterGrid = () => (
    <div className={FILTER_CONTAINER_STYLES.grid}>
      ...
      <div className='flex flex-col gap-1'>
        <label className={FILTER_CONTAINER_STYLES.label}>
          Location
        </label>
        <FilterDropdown
          label="All Locations"
  ...
  return (
    <>
      {/* Desktop View */}
      <div className={`${FILTER_CONTAINER_STYLES.main} ${className} hidden md:block`}>
        {showClearAll && (
      ...
      {isMobileSheetOpen && (
        <div className={MOBILE_STYLES.overlay} onClick={closeMobileSheet}>
          <div className={MOBILE_STYLES.sheet} onClick={(e) => e.stopPropagation()}>
            <div className={MOBILE_STYLES.sheetHeader}>
              <h2 className={MOBILE_STYLES.sheetTitle}>Filters</h2>
              <button onClick={closeMobileSheet} className={MOBILE_STYLES.closeButton}>
            ...
            <div className={MOBILE_STYLES.sheetContent}>
              ...
              {/* Sticky Apply Button */}
              <button onClick={applyMobileFilters} className={MOBILE_STYLES.applyButton}>
                Apply Filters
              </button>
            </div>
          </div>
